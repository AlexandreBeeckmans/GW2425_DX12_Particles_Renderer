...
std::shared_ptr<Scene> CommandList::CreatePlane( float width, float height, bool reverseWinding )
{
    ...
    VertexCollection vertices = {
        Vertex( XMFLOAT3( -0.5f * width, 0.5f * height, 0.0f), XMFLOAT3( 0.0f, 0.0f, -1.0f ), XMFLOAT3( 0.0f, 0.0f, 0.0f ) ),  // 0
...
std::shared_ptr<Scene> CommandList::CreatePoint()
{
    using Vertex = VertexPositionNormalTangentBitangentTexture;
    ...
    // Define a plane that is aligned with the X-Z plane and the normal is facing up in the Y-axis.
    VertexCollection vertices = {
        Vertex( XMFLOAT3( 0.0f, 0.0f, 0.0f), XMFLOAT3( 0.0f, 0.0f, -1.0f ), XMFLOAT3( 0.0f, 0.0f, 0.0f ) ),  // 0
    };
    // clang-format on
    IndexCollection indices = { 0 };

    return CreateScene( vertices, indices );
}

std::shared_ptr<dx12lib::Scene> CommandList::CreatePoints(const std::vector<XMFLOAT3>& positions)
{
    VertexCollection vertices {};
    for ( const XMFLOAT3& pos: positions)
    {
        vertices.emplace_back(VertexPositionNormalTangentBitangentTexture { pos, XMFLOAT3( 0, 0, -1 ), XMFLOAT3( 0, 0, 0 ) } );
    }
                               ...
                               uint32_t startInstance )
{
    ...
    //m_d3d12CommandList->Dispatch( 1, 1, 1 );
}
...
void CommandList::MeshShaderDraw(int nbX)
{
    FlushResourceBarriers();
    ...
    m_d3d12CommandList->SetGraphicsRootSignature( m_RootSignature );
    m_d3d12CommandList->DispatchMesh( nbX, nbY, 1 );
}
...
void CommandList::InvertNormals( VertexCollection& vertices )
{
